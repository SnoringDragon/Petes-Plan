...
class BoilerGradesService extends Api {
    getCourse(options: { courseID: string, subject: string } | { course: string }) {
        return this.get<Boilergrade[]>(`/api/boilergrades/course?${new URLSearchParams(options)}`);
    }

    getInstructor(options: { email: string } | { instructor: string }) {
        return this.get<Boilergrade[]>(`/api/boilergrades/instructor?${new URLSearchParams(options)}`);
    }
